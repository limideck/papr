...
impl AiConfig {
    ...
    pub fn new(
        ...
        //
        // The key is optional: local OpenAI-compatible servers (LM Studio,
        // Ollama, llama.cpp, a proxy) routinely run without auth, and forcing
        // one here would block every AI feature for them. When empty, the
        // request builders simply omit the auth header.
        let api_key = api_key.map(|k| k.trim().to_string()).unwrap_or_default();
...
mod tests {
    ...
    #[test]
    fn config_allows_a_keyless_local_endpoint() {
        // Local OpenAI-compatible servers often run without auth. A blank
        // key must still build a config (rather than aborting with `noAiKey`);
        // the request builders then omit the auth header entirely.
        let cfg = AiConfig::new(Some("openai".into()), Some("   \n".into()), None, None)
            .expect("a blank key is valid for a keyless endpoint");
