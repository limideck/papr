...
/// per batch (a group of whole blocks), never per token: token-level events
/// across a full article would flood the client and freeze the UI.
...
pub enum TranslateEvent {
    ...
    Done { html: String },
    /// A provider / network failure mid-stream. Surfaced so the web SSE client
    /// can toast the message (HTTP already returned 200 when the stream opened).
    Error(String),
}
