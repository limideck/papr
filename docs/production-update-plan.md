...
> 制定：2026-09（本地收尾后）
> 状态：**✅ 已执行 2026-09-08**（v0.1.25：commit 0f727af…f97c5dd，release run 34208767652 成功；迁移 v33→v35；meili 重建 35,225 docs 对账完成；回滚物 bin/*.old-v0124 + dist.old-v0124 + /root/backups 保留）。下节为执行前版本，保留作复盘。
...
- [x] 已执行：提交收拢并推送 → 本地 v33→v35 迁移演练通过 → tag v0.1.25 → GH release 构建成功 → 部署 + 验证清单全绿 → meili 全量重建对账一致
